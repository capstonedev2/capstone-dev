...
export function AdviserSubmissionReviewWorkspace({ fileId }: { fileId: string }) {
  ...
  async function sendReminder() {
    ...
    try {
      ...
      setError(reminderError instanceof Error ? reminderError.message : 'Unable to send reminder.');
    } finally {
      setIsSaving(false);
    }
  }
      ...
      <div className="mx-auto max-w-[1800px] space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-black text-orange-700 ring-1 ring-inset ring-orange-100">{getDeadlineLabel(submission.deadline)}</span>
          </div>
        </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px] xl:items-center">
              <div className="min-w-0">
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="rounded-2xl bg-white/10 p-3 ring-1 ring-inset ring-white/15">
                    <p className="text-[10px] font-black uppercase tracking-wide text-blue-100">Deadline</p>
                    <p className="mt-1 text-sm font-black">{formatSubmissionDate(submission.deadline)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row xl:justify-end">
                ...
                <button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-white/25 bg-white/10 px-5 text-sm font-black text-white transition hover:bg-white/15" type="button" onClick={() => document.getElementById('adviser-document-details')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>
                  <i className="fas fa-circle-info text-xs" aria-hidden="true" />
                  View Details
                </button>
              </div>
            </div>
              ...
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
                <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                  <div className="flex flex-wrap gap-5 border-b border-slate-100">
                    {['Adviser Notes', 'Version History', 'Document Details'].map((tab) => (
                      <button key={tab} className={`pb-3 text-xs font-black uppercase tracking-[0.12em] ${tab === 'Adviser Notes' ? 'border-b-2 border-[#003A8F] text-[#003A8F]' : 'text-slate-500'}`} type="button">
                        {tab}
                      </button>
                    ))}
                  </div>
                ...
                <section id="adviser-document-details" className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
          ...
          <aside className="space-y-5">
                ...
                <>
                  <div>
                    ...
                    <p className="mt-1 text-xs font-bold leading-5 text-slate-500">
                      Saved comments stay in the adviser review until you request revision or approve the submission.
                    </p>
                  </div>
                  ...
                  {isUnderReview ? (
                    <div className="mt-4 rounded-2xl border border-orange-200 bg-orange-50 p-4">
                      <div className="flex items-start gap-3">
                        <span className="relative mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-orange-500 text-white">
                          <span className="absolute inset-0 rounded-full bg-orange-400 opacity-30 animate-ping" />
                          <i className="fas fa-circle relative text-[8px]" aria-hidden="true" />
                        </span>
                        <div>
                          <p className="text-sm font-black text-orange-900">Review session active</p>
                          <p className="mt-1 text-xs font-bold leading-5 text-orange-800">
                            Adviser currently reviewing this document. Review started on {formatSubmissionDateTime(reviewStartedAt)}.
                          </p>
                        </div>
                      </div>
                    </div>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-start justify-between gap-3">
                                  ...
                                  <button className="text-slate-400 transition hover:text-[#003A8F]" type="button" aria-label="Comment options">
                                ...
                                <button className="mt-3 text-xs font-black text-[#003A8F]" type="button">Reply</button>
                              </div>
                            </div>
                          </article>
                        );
                  ...
                  <button className="mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 text-sm font-black text-[#003A8F] transition hover:bg-blue-100" type="button" onClick={() => document.getElementById('adviser-review-note')?.focus()}>
            ...
            <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-[0_18px_42px_rgba(15,23,42,0.06)]">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[11px] font-black uppercase tracking-[0.15em] text-[#003A8F]">Version History</p>
                  <h2 className="mt-1 text-lg font-black text-slate-950">All Versions</h2>
                </div>
