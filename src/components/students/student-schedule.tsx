...
const TYPE_STYLES: Record<ScheduleItemType, string> = {
  ...
  Event: 'bg-[var(--surface-alt)] text-[var(--text)]',
...
const PRIORITY_STYLES: Record<ScheduleItemPriority, string> = {
  ...
  low: 'bg-[var(--surface-alt)] text-[var(--muted)] ring-slate-200'
};

const PRIMARY_SURFACE = 'rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm sm:p-6';
...
function EmptyState({ copy = 'No schedule items available' }: { copy?: string }) {
  return (
    <div className="flex min-h-[140px] items-center justify-center rounded-xl bg-[var(--surface-alt)] px-6 py-10 text-center text-sm font-medium text-[var(--muted)]">
      {copy}
    </div>
  );
}
...
function Meta({ icon, value }: { icon: string; value?: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 text-xs font-medium text-[var(--muted)]">
      <i className={`fas ${icon} text-[11px] text-[var(--text-meta)]`} aria-hidden="true" />
      {value}
    </span>
  );
}
...
function SummaryCard({
  ...
  return (
    <article className={cx('group relative overflow-hidden rounded-lg border border-[var(--border)] p-4 shadow-sm transition-all duration-200 hover:border-[var(--border-strong)] hover:shadow-md sm:p-5', className)}>
      <div className="relative flex items-start justify-between gap-4">
        <div className="space-y-1">
          <span className={cx('text-[11px] font-bold uppercase tracking-[0.18em]', labelClassName)}>{label}</span>
          <strong className="block text-3xl font-extrabold tracking-tight text-[var(--text)]">{value}</strong>
        </div>
      ...
      <p className="relative mt-3 text-[13px] font-medium leading-relaxed text-[var(--muted)]">{description}</p>
    </article>
  );
}
...
function EventItem({
  ...
  return (
    <article className={cx('group relative overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface)] shadow-sm transition-all duration-200 hover:border-[var(--border-strong)] hover:shadow-md', className)}>
      ...
      <div className="grid gap-4 p-4 sm:grid-cols-[116px_minmax(0,1fr)] sm:p-5">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-alt)] px-3 py-3 text-left">
          <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--muted)]">{item.dateLabel}</p>
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--text-meta)]">{getScheduleSourceLabel(item)}</span>
          </div>
        </div>
        ...
        <div className="min-w-0">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
            <div className="min-w-0 space-y-2.5">
              ...
              <div className="space-y-1.5">
                ...
                <p className={cx('text-[13px] font-medium text-[var(--muted)]', compact ? 'leading-relaxed line-clamp-2' : 'leading-relaxed')}>{item.description}</p>
              </div>
            </div>

            {!compact ? (
              <Link prefetch={false}
                className="inline-flex min-h-9 shrink-0 items-center justify-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 text-xs font-bold text-[var(--text)] shadow-sm transition-all duration-200 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-[var(--border)] pt-3">
...
function TimelineItem({
  ...
  return (
    <div className="relative pl-7">
      ...
      <span className={cx('absolute left-[5px] top-8 flex h-[18px] w-[18px] items-center justify-center rounded-full bg-[var(--surface)] shadow-sm ring-1 ring-slate-200')} aria-hidden="true">
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1.5">
          <span className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--text-meta)]">{eyebrow}</span>
...
export function StudentSchedule({ data }: { data: StudentDashboardData }) {
  ...
  const plannerMetrics = [
    ...
    },
    {
      ...
      accentClassName: 'border-[var(--border)]'
    },
    {
  ...
  const timelineSections = [
    {
      ...
      badgeClassName: model.completedItems.length ? 'bg-emerald-100 text-emerald-700' : 'bg-[var(--surface-alt)] text-[var(--text)]',
      icon: 'fa-circle-check'
    }
  ] as const;
    ...
    <div className="student-schedule-page">
      <header className="top-nav student-schedule-header">
        <div className="top-nav-leading w-full">
          <div className="flex w-full flex-col gap-6 border-b border-[var(--border)] pb-6">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div className="page-title student-schedule-header-copy space-y-3">
                <div className="page-title-context student-schedule-header-context flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">
        ...
        <section className="flex flex-col gap-6">
          <div className="space-y-6">
              ...
              <div className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.75fr)]">
                <div className="min-w-0 space-y-5">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="space-y-1.5">
                      <span className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--text-meta)]">Command Center</span>
                      <h2 className="text-2xl font-semibold tracking-tight text-slate-950">
                        {model.nextUpcomingEvent ? model.nextUpcomingEvent.title : 'No active schedule item'}
                      </h2>
                      <p className="max-w-3xl text-sm leading-6 text-[var(--muted)]">{model.plannerNote}</p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <Pill label={`${model.todayItems.length} today`} icon="fa-clock" className={model.todayItems.length ? 'bg-yellow-100 text-amber-900' : 'bg-[var(--surface-alt)] text-[var(--text)]'} />
                      <Pill label={`${model.summary.overdueMilestones} overdue`} icon="fa-triangle-exclamation" className={model.summary.overdueMilestones ? 'bg-rose-100 text-rose-700' : 'bg-[var(--surface-alt)] text-[var(--text)]'} />
                      <Pill label={`${adviserItems.length} adviser-set`} icon="fa-user-tie" className={adviserItems.length ? 'bg-blue-100 text-blue-700' : 'bg-[var(--surface-alt)] text-[var(--text)]'} />
                    </div>
                  </div>

                  {model.nextUpcomingEvent ? (
                    <EventItem item={model.nextUpcomingEvent} className="bg-[var(--surface-alt)] shadow-none hover:shadow-sm" />
                  ) : (
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                    {plannerMetrics.map((metric) => (
                      <div key={metric.label} className={cx('rounded-lg border bg-[var(--surface)] px-4 py-3', metric.accentClassName)}>
                        <div className="flex items-start justify-between gap-3">
                          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--text-meta)]">{metric.label}</p>
                          <strong className="text-xl font-semibold tracking-tight text-slate-950">{metric.value}</strong>
                        </div>
                        <p className="mt-2 text-xs font-medium leading-5 text-[var(--muted)]">{metric.detail}</p>
                      </div>
                    ))}
                  </div>
                </div>

                <aside className="min-w-0 rounded-lg border border-[var(--border)] bg-[var(--surface-alt)] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      <span className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--text-meta)]">Attention</span>
                      <h3 className="text-base font-semibold tracking-tight text-slate-950">Signals and priority queue</h3>
                    </div>
                    <Pill label={`${completedPercent}% complete`} icon="fa-chart-simple" className="bg-[var(--surface)] text-[var(--text)] ring-1 ring-slate-200" />
                  </div>

                  <div className="mt-4 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-3">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--muted)]">Milestone completion</span>
                      <span className="text-xs font-bold text-[var(--text)]">{model.summary.completedMilestones}/{model.summary.totalMilestones}</span>
                    </div>
                  ...
                  <div className="mt-4 space-y-2 border-t border-[var(--border)] pt-4">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--muted)]">Priority queue</span>
                      <span className="text-xs font-semibold text-[var(--muted)]">{priorityQueue.length} active</span>
                    </div>
                      ...
                      <div className="space-y-2">
                        {priorityQueue.map((item) => (
                          <Link prefetch={false}
                            key={item.id}
                            href={getScheduleActionHref(item)}
                            className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2.5 text-left transition hover:border-blue-200 hover:bg-blue-50"
                          >
                            ...
                            <span className="min-w-0">
                              ...
                              <span className="mt-0.5 block truncate text-xs font-medium text-[var(--muted)]">{item.dateLabel} - {item.timeLabel}</span>
                            </span>
                            <i className="fas fa-chevron-right text-[10px] text-[var(--text-meta)]" aria-hidden="true" />
                          </Link>
                        ))}
                      </div>
                    ) : (
            ...
            <article className={PRIMARY_SURFACE}>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-1.5">
                  <span className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--text-meta)]">Weekly Planner</span>
                  <h3 className="text-lg font-semibold tracking-tight text-[var(--text)]">Week of {formatWeekRangeLabel(model.weekStart, model.weekEnd)}</h3>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Pill label={`${weekDaysWithItems}/7 active`} icon="fa-layer-group" className="bg-[var(--surface-alt)] text-[var(--text)]" />
                </div>
              </div>
                ...
                {model.weeklyPlannerCells.map((cell) => (
                  <article
                    ...
                    className={cx(
                      ...
                      cell.isToday
                        ? 'border-blue-200 bg-[var(--surface)] shadow-[0_4px_18px_rgb(59,130,246,0.10)] ring-1 ring-blue-100'
                        : 'border-[var(--border)] bg-[var(--surface-alt)] hover:border-[var(--border-strong)] hover:bg-[var(--surface)] hover:shadow-sm'
                    )}
                  >
                    {cell.isToday && (
                    ...
                    <div className="relative flex items-start justify-between gap-3 border-b border-[var(--border)] pb-3">
                      <div className="flex flex-col">
                        <span className={cx('text-[9px] font-bold uppercase tracking-[0.2em]', cell.isToday ? 'text-blue-600' : 'text-[var(--text-meta)]')}>{formatWeekdayLabel(cell.day)}</span>
                        <strong className={cx('block text-base font-extrabold tracking-tight', cell.isToday ? 'text-blue-950' : 'text-[var(--text)]')}>{formatMonthDayLabel(cell.day)}</strong>
                      </div>
                      <div className="flex flex-col items-end gap-1.5">
                        ...
                        {cell.items.length ? (
                          <span className={cx('inline-flex h-5 items-center justify-center rounded px-2 text-[10px] font-bold', cell.isToday ? 'bg-blue-100 text-blue-700' : 'bg-[var(--surface-alt)] text-[var(--muted)]')}>
                    ...
                    <div className="relative mt-3 flex-1 space-y-2">
                      {cell.items.length ? (
                        cell.items.map((item) => {
                          ...
                          return (
                            <Link prefetch={false}
                              key={item.id}
                              href={getScheduleActionHref(item)}
                              className="group/cell relative flex gap-2.5 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-2.5 shadow-sm transition-all duration-200 hover:border-blue-200 hover:bg-blue-50 hover:shadow-md hover:ring-1 hover:ring-blue-100"
                            >
                              <span className={cx('mt-1 flex h-2 w-2 shrink-0 rounded-full', tone.dot)} aria-hidden="true" />
                              <div className="min-w-0">
                                <p className="text-[12px] font-bold leading-tight text-[var(--text)] transition-colors group-hover/cell:text-blue-700">{item.title}</p>
                                <p className="mt-1 flex items-center gap-1 text-[10px] font-medium text-[var(--muted)]">
                                  <i className="fas fa-clock text-[8px] text-[var(--text-meta)]" /> {item.timeLabel}
                                </p>
                              </div>
                            </Link>
                          );
                        ...
                        <div className="flex h-full flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-[var(--border)] bg-[var(--surface-alt)] p-3 text-center transition-colors group-hover:bg-[var(--surface-alt)]">
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--surface)] shadow-sm ring-1 ring-slate-100 text-[var(--muted)]">
                            <i className="fas fa-calendar-minus text-xs" />
                          </div>
                          <span className="block text-[9px] font-bold uppercase tracking-[0.15em] text-[var(--text-meta)]">Open Day</span>
                        </div>
                      )}
                    </div>
                  </article>
            ...
            <article className={PRIMARY_SURFACE}>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-1.5">
                  <span className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--text-meta)]">Active Agenda</span>
                  <h3 className="text-lg font-semibold tracking-tight text-[var(--text)]">Filtered schedule items</h3>
                  <p className="max-w-2xl text-sm leading-6 text-[var(--muted)]">
                    Adviser-created consultations, deadlines, reminders, and generated milestone dates stay in one list.
                  </p>
                </div>
                <Pill label={`${filteredItems.length} shown`} icon="fa-list" className="bg-[var(--surface-alt)] text-[var(--text)]" />
              </div>
                    ...
                    <button
                      ...
                      className={cx(
                        ...
                        active
                          ...
                          : 'border-[var(--border)] bg-[var(--surface)] text-[var(--muted)] hover:bg-[var(--surface-alt)] hover:text-[var(--text)]'
                      )}
                    >
                      <i className={`fas ${option.icon} text-xs`} aria-hidden="true" />
                      {option.label}
                      <span className={cx('ml-1 inline-flex h-5 min-w-5 items-center justify-center rounded px-1.5 text-[11px] font-bold', active ? 'bg-[var(--surface)] text-white' : 'bg-[var(--surface-alt)] text-[var(--muted)]')}>
                ...
                {timelineSections.map((section, index) => (
                  <div key={section.id} className="space-y-8">
                    <TimelineSection
                      ...
                      icon={section.icon}
                    />
                    {index !== timelineSections.length - 1 ? <div className="h-px bg-[var(--surface-alt)]" /> : null}
                  </div>
                ))}
              </div>
            </article>
